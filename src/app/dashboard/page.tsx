...
import db from "@/db";
import { checkIsAdmin, formatPrice } from "@/lib/utils";
...
async function page() {
  ...
  const lastWeeksSum = await db.order.aggregate({
    where: { isPaid: true, createdAt: { gte: new Date(new Date().getDate() - 7) } },
    _sum: { amount: true },
  });
  const lastMonthSum = await db.order.aggregate({
    where: { isPaid: true, createdAt: { gte: new Date(new Date().getDate() - 30) } },
    _sum: { amount: true },
  });

  const WEEKLY_GOAL = 200;
  const MONTHLY_GOAL = 800;
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-4xl">
                  {formatPrice(lastWeeksSum._sum.amount ?? 0)}
                </CardTitle>
                <CardContent>
                ...
                <CardFooter>
                  <Progress value={((lastWeeksSum._sum.amount ?? 0) * 100) / WEEKLY_GOAL} />
                </CardFooter>
              </CardHeader>
            </Card>
